...
public class UDTFMatrixProfileAnomaly implements UDTF {

    private int m; // 窗口长度
    private int k; // 相似度计算阈值
    private double percentileThreshold; // 异常百分数
    ...
    public void validate(UDFParameterValidator validator) throws Exception {
        validator.validateInputSeriesDataType(0, Type.DOUBLE)
                ...
                .validate(x -> (double) x >= 0 && (double) x <= 100,
                        "Percentile threshold must be between 0 and 100.",
                        validator.getParameters().getDoubleOrDefault("percentile", 95.0));
    }
    ...
    public void beforeStart(UDFParameters parameters, UDTFConfigurations configurations) throws Exception {
        ...
        this.percentileThreshold = parameters.getDoubleOrDefault("percentile", 95.0);
        configurations
    ...
    public void terminate(PointCollector collector) throws Exception {
        List<List<Double>> matrixProfileResult = Stamp.computeMatrixProfile(values, this.m, this.k,
                Stamp.DEFAULT_VARIANCE_THRESHOLD);
        List<Double> mp = matrixProfileResult.get(0);
        List<Integer> anomalyIndices = Stamp.detectAnomalies(mp, this.percentileThreshold);
        ...
        for (int i = 0; i < anomalyIndices.size(); i++) {
            for (int j = 0; j < m; j++) {
                anomalies.set(anomalyIndices.get(i) + j, true);
            }
        }
        for (int i = 0; i < anomalies.size(); i++) {
            collector.putBoolean(timestamps.get(i), anomalies.get(i));
        }
    }
}
